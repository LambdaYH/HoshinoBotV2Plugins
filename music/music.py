...
@sv.on_command("music", aliases=["点歌", "音乐", "来首歌"], only_to_me=False)
...
@sv.on_natural_language(keywords=CALLING_KEYWORDS, only_to_me=False)
async def _(session: NLPSession):
    sp = re.split("|".join(CALLING_KEYWORDS), session.msg_text, maxsplit=1)
    if sp:
        return IntentCommand(
            90.0, "music", args={"keyword": sp[-1].strip(" 吧呗"), "from_nlp": True}
        )
